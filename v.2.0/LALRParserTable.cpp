...
namespace tinyYACC {
	LALRParserTable::LALRParserTable(Grammar &g) :
		g_(g)
	{
	}
	...
	//
	void LALRParserTable::computerLookAhead()
	{
	...
	size_t LALRParserTable::expandRule(list<Item>& items, bool& exists)
	{
	...
	//
	void LALRParserTable::printStats()
	{
		for (auto pr : status_) {
	...
	LALRParserTable::Action LALRParserTable::makeAction(Action::action act, size_t target)
	{
		assert(act == Action::shift);
		return Action(Action::shift, target);
	}

	LALRParserTable::Action LALRParserTable::makeAction(Action::action act, rulePtr& r)
	{
